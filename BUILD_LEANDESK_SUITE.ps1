...
$Venv = Join-Path $env:TEMP "LeanDesk_0.9.0_Office_Completion_Build_Venv"
...
function Get-Existing-CodeSigningCertificate {
    ...
    if (-not $cert) { throw "Existing Dietrich AI Labs code-signing certificate with private key was not found." }
    return $cert
}

function Write-ChecksumManifest([string]$PackageRoot) {
...
$required = @(
    ...
    "tests\test_hotfix_081.py", "tests\test_windows_lifecycle_hold_correction.py"
)
...
$GateEvidenceRoot = Join-Path $env:LOCALAPPDATA "LeanDeskBuildEvidence\0.9.0-office-completion\$RunId"
...
Write-Step "Sign application with the existing Dietrich AI Labs certificate"
$SignTool = Find-SignTool
if (-not $SignTool) { throw "Windows SDK SignTool was not found; unsigned output is forbidden." }
$Cert = Get-Existing-CodeSigningCertificate
...
$Installer = Join-Path $ReleaseRoot "LeanDesk_Suite_Setup_0.9.0.exe"
...
Require-File $GateAuthorization
$Portable = Join-Path $ReadyRoot "LeanDesk_Suite_Portable_0.9.0.zip"
$InstallerZip = Join-Path $ReadyRoot "LeanDesk_Suite_Installer_0.9.0.zip"
$Complete = Join-Path $ReadyRoot "LeanDesk_Suite_Complete_0.9.0.zip"
$PackageStageRoot = Join-Path $GateEvidenceRoot "package-staging"
$PortableStage = Join-Path $PackageStageRoot "portable"
$InstallerStage = Join-Path $PackageStageRoot "installer"
New-Item $PortableStage, $InstallerStage -ItemType Directory -Force | Out-Null
...
foreach ($path in @(
    ...
    $PublicCert
)) { Copy-Item -LiteralPath $path -Destination $PortableStage }
Write-ChecksumManifest $PortableStage
Compress-Archive -Path (Join-Path $PortableStage "*") -DestinationPath $Portable -Force
...
foreach ($path in @(
    ...
    (Join-Path $ReleaseRoot "README.md"),
    (Join-Path $ReleaseRoot "EULA.txt")
)) { Copy-Item -LiteralPath $path -Destination $InstallerStage }
Write-ChecksumManifest $InstallerStage
