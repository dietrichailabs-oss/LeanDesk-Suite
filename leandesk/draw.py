...
class Shape:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "Shape":
        ...
        known = {"shape_id", "kind", "x1", "y1", "x2", "y2", "fill", "outline", "width", "text", "extra"}
...
class DrawFrame(ttk.Frame):
    ...
    def highlight_selected(self) -> None:
        ...
        if shape.kind == "text":
            x2 = x1 + max(80, abs(shape.x2 - shape.x1)); y2 = y1 + 45
    ...
    def delete_selected(self) -> None:
        if self.selected_id:
            self.drawing.shapes = [shape for shape in self.drawing.shapes if shape.shape_id != self.selected_id]
