...
from .workspace import SearchResult, TemplateStore, search_records
...
def initial_window_bounds(screen_width: int, screen_height: int) -> tuple[int, int, int, int]:
    """Return a centered startup window that stays inside the visible desktop."""
    width = min(DEFAULT_WINDOW_WIDTH, max(1, screen_width - WINDOW_HORIZONTAL_MARGIN))
    height = min(DEFAULT_WINDOW_HEIGHT, max(1, screen_height - WINDOW_VERTICAL_MARGIN))
...
class LeanDeskApp(tk.Tk):
    ...
    def _build_menu(self) -> None:
        ...
        tools.add_command(label="Global Search...", command=self.open_global_search)
        tools.add_command(label="New from Template...", command=self.new_from_template)
    ...
    def _search_rows(self) -> list[SearchResult]:
        rows = [SearchResult(entry.module, entry.display_name or Path(entry.path).name, entry.path, entry.path) for entry in self.recent.entries]
        notes = self.frames["Notes"]
        rows.extend(SearchResult("Notes", note.title, f"{note.notebook} {note.tags} {note.body}", note.note_id) for note in notes.notes)
        tasks = self.frames["Tasks"]
        rows.extend(SearchResult("Tasks", item.title, f"{item.project} {item.status} {item.categories} {item.notes}", item.task_id) for item in tasks.tasks)
        calendar = self.frames["Calendar"]
        rows.extend(SearchResult("Calendar", item.title, f"{item.date} {item.time} {item.location} {item.notes}", item.event_id) for item in calendar.events)
        contacts = self.frames["Contacts"]
        rows.extend(SearchResult("Contacts", item.name, f"{item.company} {item.email} {item.phone} {item.categories} {item.notes}", item.contact_id) for item in contacts.contacts)
        return rows

    def open_global_search(self) -> None:
        query = simpledialog.askstring("Global Search", "Search documents, notes, tasks, events, and contacts:", parent=self)
        if query is None:
            return
        results = search_records(query, self._search_rows())
        window = tk.Toplevel(self); window.title("LeanDesk Global Search"); window.geometry("900x520")
        tree = ttk.Treeview(window, columns=("module", "title", "detail"), show="headings")
        for name, width in (("module", 100), ("title", 240), ("detail", 520)):
            tree.heading(name, text=name.title()); tree.column(name, width=width)
        for index, row in enumerate(results):
            tree.insert("", "end", iid=f"result-{index}", values=(row.module, row.title, row.detail[:500]))
        tree.pack(fill="both", expand=True, padx=12, pady=12)

    def new_from_template(self) -> None:
        templates = TemplateStore().list()
        names = ", ".join(template.name for template in templates)
        selected = simpledialog.askstring("New from Template", f"Template name:\n\n{names}", parent=self)
        template = next((row for row in templates if row.name.casefold() == (selected or "").strip().casefold()), None)
        if not template:
            return
        self.show_module(template.module, new=True)
        frame = self.frames[template.module]
        if template.module == "Writer":
            frame.text.insert("1.0", str(template.payload.get("text", "")))
        elif template.module == "Sheets":
            for col, value in enumerate(template.payload.get("headers", [])):
                from .sheets import column_name
                frame.active_sheet().set(f"{column_name(col)}1", str(value))
            frame.active_grid().refresh(); frame.mark_dirty()
        elif template.module == "Slides":
            frame.title_var.set(str(template.payload.get("title", ""))); frame.body_text.insert("1.0", str(template.payload.get("body", ""))); frame.form_changed()
        elif template.module == "Notes":
            frame.title_var.set(str(template.payload.get("title", ""))); frame.editor_text.insert("1.0", str(template.payload.get("body", ""))); frame.editor_changed()
