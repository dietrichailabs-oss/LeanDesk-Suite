...
class WriterFrame(ttk.Frame):
    ...
    def _build_insert_ribbon(self) -> None:
        ...
        actions = (
            ...
            ("Text Table", self.insert_text_table),
        )
        ...
        self._tab_button(media, "Image Reference", self.insert_image_reference, width=16).pack(
    ...
    def insert_text_table(self) -> None:
        rows = simpledialog.askinteger("Insert Text Table", "Rows:", initialvalue=3, minvalue=1, maxvalue=20, parent=self)
        if rows is None:
            return
        cols = simpledialog.askinteger("Insert Text Table", "Columns:", initialvalue=3, minvalue=1, maxvalue=12, parent=self)
        if cols is None:
            return
        width = 14
        border = "+" + "+".join("-" * width for _ in range(cols)) + "+\n"
        lines = [border]
        for _ in range(rows):
            lines.append("|" + "|".join(" " * width for _ in range(cols)) + "|\n")
            lines.append(border)
        self.text.insert("insert", "".join(lines))
    ...
    def insert_image_reference(self) -> None:
        ...
        if value:
            self.text.insert("insert", f"[Image: {value}]")
    ...
    def serialize(self) -> LeanDocument:
        ...
        return LeanDocument(
            ...
            tags=tags,
            metadata={"saved_at": datetime.now().isoformat(timespec="seconds"), "zoom": self.zoom},
        )
