...
class Note:
    ...
    def from_dict(cls, payload: dict) -> "Note":
        ...
        known = {"note_id", "title", "body", "notebook", "tags", "created_at", "updated_at", "pinned", "parent_id"}
        note_id = payload.get("note_id", "")
        text_fields = ("note_id", "title", "body", "notebook", "tags", "created_at", "updated_at", "parent_id")
...
class NotesFrame(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        for label, command in (
            ...
            ("Checklist", self.insert_checklist), ("Table", self.insert_table), ("Picture", self.insert_image_dialog),
            ("Export HTML", self.export_html), ("Print", self.print_note),
        ...
        ttk.Entry(meta, textvariable=self.parent_var, width=16).pack(side="left", padx=4, pady=14)
    ...
    def load_note(self, note_id: str) -> None:
        ...
        self.parent_var.set(note.parent_id)
        self.editor_text.delete("1.0", "end")
    ...
    def commit_current(self) -> None:
        ...
        note.parent_id = self.parent_var.get().strip()
    ...
    def wrap_selection(self, marker: str) -> None:
        try:
            ...
            value = self.editor_text.get(start, end)
            self.editor_text.delete(start, end)
            self.editor_text.insert(start, marker + value + marker)
        except tk.TclError:
            self.editor_text.insert("insert", marker + marker)
    ...
    @staticmethod
    def markdown_table(rows: int, cols: int) -> str:
        rows, cols = max(1, rows), max(1, cols)
        header = "| " + " | ".join(f"Column {index + 1}" for index in range(cols)) + " |"
        rule = "| " + " | ".join("---" for _ in range(cols)) + " |"
        body = ["| " + " | ".join("" for _ in range(cols)) + " |" for _ in range(rows)]
        return "\n".join([header, rule, *body]) + "\n"

    def insert_table(self) -> None:
        rows = simpledialog.askinteger("Insert Table", "Data rows:", initialvalue=3, minvalue=1, maxvalue=30, parent=self)
        cols = simpledialog.askinteger("Insert Table", "Columns:", initialvalue=3, minvalue=1, maxvalue=12, parent=self) if rows else None
        if rows and cols:
            self.editor_text.insert("insert", self.markdown_table(rows, cols))
            self.editor_changed()

    def insert_image_dialog(self) -> None:
        value = filedialog.askopenfilename(parent=self, filetypes=(("Images", "*.png *.jpg *.jpeg *.gif *.webp"),))
        if value:
            self.insert_image(Path(value))

    def insert_image(self, path: Path) -> None:
        data = read_bounded(Path(path), limit=8 * 1024 * 1024)
        suffix = Path(path).suffix.lower()
        media = {".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".gif": "image/gif", ".webp": "image/webp"}.get(suffix)
        if not media:
            raise ValueError("Unsupported note image format")
        encoded = base64.b64encode(data).decode("ascii")
        self.editor_text.insert("insert", f"![{Path(path).stem}](data:{media};base64,{encoded})")
        self.editor_changed()

    def export_html(self) -> None:
        value = filedialog.asksaveasfilename(parent=self, defaultextension=".html", filetypes=(("HTML", "*.html"),))
        if not value:
            return
        self.commit_current()
        note = self.get_note(self.current_id)
        if note:
            from .document_formats import plain_to_html
            atomic_write_text(Path(value), plain_to_html(note.body, note.title))
            self.status_var.set(f"Exported {Path(value).name}")
    ...
    def print_note(self) -> None:
        ...
        if note:
            from .document_formats import plain_to_html
            target = Path(tempfile.gettempdir()) / "LeanDesk_Note_Print.html"
            atomic_write_text(target, plain_to_html(note.body, note.title))
