...
class TasksFrame(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        for label, var, values in (("Title", self.title_var, None), ("Due YYYY-MM-DD", self.due_var, None), ("Priority", self.priority_var, ("Low", "Normal", "High", "Critical")), ("Status", self.status_edit_var, ("Open", "In Progress", "Waiting", "Completed")), ("Project", self.project_var, None), ("Recurrence", self.recurrence_var, ("", "Daily", "Weekly", "Monthly", "Yearly")), ("Reminder", self.reminder_var, None), ("Categories", self.categories_var, None)):
    ...
    def refresh(self, select_id=None):
        ...
        if needle: rows = [item for item in rows if needle in "\n".join((item.title, item.project, item.notes, item.status, item.categories, item.recurrence)).lower()]
    ...
    def on_select(self, _event=None):
        ...
        if not task: return
        self.current_id = task.task_id; self.title_var.set(task.title); self.due_var.set(task.due); self.priority_var.set(task.priority); self.status_edit_var.set(task.status); self.project_var.set(task.project); self.recurrence_var.set(task.recurrence); self.reminder_var.set(task.reminder); self.categories_var.set(task.categories)
    ...
    def _load_direct(self, task):
        self.title_var.set(task.title); self.due_var.set(task.due); self.priority_var.set(task.priority); self.status_edit_var.set(task.status); self.project_var.set(task.project); self.recurrence_var.set(task.recurrence); self.reminder_var.set(task.reminder); self.categories_var.set(task.categories); self.notes_text.delete("1.0", "end"); self.notes_text.insert("1.0", task.notes)
    def save_current(self):
        task = self.get(self.current_id)
        if not task: self.new_task(); task = self.get(self.current_id)
        task.title = self.title_var.get().strip() or "Untitled Task"; task.due = self.due_var.get().strip(); task.priority = self.priority_var.get(); task.status = self.status_edit_var.get(); task.project = self.project_var.get().strip(); task.recurrence = self.recurrence_var.get().strip(); task.reminder = self.reminder_var.get().strip(); task.categories = self.categories_var.get().strip(); task.notes = self.notes_text.get("1.0", "end-1c").strip(); self.save_all(); self.refresh(task.task_id); self.status_var.set(f"Saved {task.title}")
...
class CalendarFrame(ttk.Frame):
    ...
    def add_event(self):
        ...
        if not title: return
        time_value = simpledialog.askstring("Event time", "Start time (blank for all day):", parent=self) or ""; end_time = simpledialog.askstring("Event time", "End time (optional):", parent=self) or ""; location = simpledialog.askstring("Event location", "Location (optional):", parent=self) or ""; recurrence = simpledialog.askstring("Recurrence", "None, Daily, Weekly, Monthly, or Yearly:", parent=self) or ""; notes = simpledialog.askstring("Event notes", "Notes (optional):", parent=self) or ""
        self.events.append(CalendarEvent(str(uuid.uuid4()), self.selected_date.isoformat(), title.strip(), time_value.strip(), notes.strip(), end_time.strip(), "true" if not time_value.strip() else "false", recurrence.strip(), "", location.strip(), "")); self.save(); self.render_month()
...
class ContactsFrame(ttk.Frame):
    ...
    def refresh(self, select_id=None):
        ...
        if needle: rows = [item for item in rows if needle in "\n".join((item.name, item.company, item.email, item.phone, item.notes, item.job_title, item.website, item.categories)).lower()]
    ...
    def on_select(self, _event=None):
        ...
        if not item:return
        self.current_id=item.contact_id; self.name_var.set(item.name); self.company_var.set(item.company); self.email_var.set(item.email); self.phone_var.set(item.phone); self.address_var.set(item.address); self.job_title_var.set(item.job_title); self.website_var.set(item.website); self.birthday_var.set(item.birthday); self.categories_var.set(item.categories); self.notes_text.delete("1.0","end"); self.notes_text.insert("1.0",item.notes)
    ...
    def save_current(self):
        ...
        item.name=self.name_var.get().strip() or "Unnamed Contact";item.company=self.company_var.get().strip();item.email=self.email_var.get().strip();item.phone=self.phone_var.get().strip();item.address=self.address_var.get().strip();item.job_title=self.job_title_var.get().strip();item.website=self.website_var.get().strip();item.birthday=self.birthday_var.get().strip();item.categories=self.categories_var.get().strip();item.notes=self.notes_text.get("1.0","end-1c").strip();self.save_all();self.refresh(item.contact_id);self.status_var.set(f"Saved {item.name}")
