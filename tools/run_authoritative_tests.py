...
REQUIRED_TEST_FILES = (
    ...
    "tests/test_correction_6_qa.py",
        ...
        "tests/test_office_completion.py",
        "tests/test_spreadsheet_features.py",
        "tests/test_organizer_exchange.py",
...
PER_TEST_PROCESS_FILES = {"tests/test_hotfix_081.py"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    execution_targets: list[tuple[Path, list[str]]] = []
    for relative in test_files:
        portable = relative.as_posix()
        if portable == CORRECTION_2_GUI_FILE:
            execution_targets.append((relative, [portable, "-k", f"not {CORRECTION_2_GUI_TEST}"]))
            collection_target = [portable, "-k", CORRECTION_2_GUI_TEST]
        elif portable in PER_TEST_PROCESS_FILES:
            collection_target = [portable]
        else:
            execution_targets.append((relative, [str(relative)]))
            continue
        node_collection = _run(
            [sys.executable, "-m", "pytest", "--collect-only", "-q", *collection_target, "--basetemp", str(execution_basetemp / "node-collection")],
            root,
        )
        if node_collection.returncode != 0:
            print(node_collection.stdout, end="")
            report["test_exit_code"] = node_collection.returncode
            return finish(node_collection.returncode)
        nodes = []
        marker = portable + "::"
        for line in node_collection.stdout.splitlines():
            candidate = line.strip().replace("\\", "/")
            position = candidate.find(marker)
            if position >= 0:
                nodes.append(candidate[position:])
        if not nodes:
            print(f"No per-test nodes collected for {portable}.", file=sys.stderr)
            return finish(11)
        execution_targets.extend((relative, [node]) for node in nodes)

    for index, (relative, target_args) in enumerate(execution_targets):
        target = " ".join(target_args)
        safe_name = re.sub(r"[^A-Za-z0-9_.-]", "_", target)[:120]
        shard_temp = execution_basetemp / f"{index:03d}-{safe_name}"
        shard = _run(
            [sys.executable, "-m", "pytest", "-q", *target_args, "--basetemp", str(shard_temp)],
            root,
        )
